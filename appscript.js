...
// Etkinlik bilgileri (gerekirse duzenleyebilirsin)
const ETKINLIK_ADI = 'Tavariş';
const ETKINLIK_YER = 'Ankara Yeni Sahne';
const ETKINLIK_ADRES = 'Kızılay, Menekşe-1 Sk. 8-A Çankaya/Ankara';

// Yeni formdaki gunler (ekrandaki seceneklere gore)
const ETKINLIK_GUN_1_TOKEN = '6';                 // "6 Mart Cuma"
const ETKINLIK_GUN_1_LABEL = '6 Mart Cuma';
const ETKINLIK_GUN_1_SAAT = '19.30';

const ETKINLIK_GUN_2_TOKEN = '9';                 // "9 Mart Pazartesi"
const ETKINLIK_GUN_2_LABEL = '9 Mart Pazartesi';
...
// Kontenjan limitleri (kisi siniri 270)
const KONTENJAN_GUN_1 = 270;
const KONTENJAN_GUN_2 = 270;
...
function getGunConfig_(gun) {
    ...
    if (gunStr.indexOf(normalizeText_(ETKINLIK_GUN_1_TOKEN)) > -1 || gunStr.indexOf(normalizeText_(ETKINLIK_GUN_1_LABEL)) > -1) {
        return {
            key: 'gun1',
            token: ETKINLIK_GUN_1_TOKEN,
            label: ETKINLIK_GUN_1_LABEL,
            saat: ETKINLIK_GUN_1_SAAT,
            limit: KONTENJAN_GUN_1,
            statCol: STAT_COL_GUN_1
        };
    }

    if (gunStr.indexOf(normalizeText_(ETKINLIK_GUN_2_TOKEN)) > -1 || gunStr.indexOf(normalizeText_(ETKINLIK_GUN_2_LABEL)) > -1) {
        return {
            key: 'gun2',
            token: ETKINLIK_GUN_2_TOKEN,
            label: ETKINLIK_GUN_2_LABEL,
            saat: ETKINLIK_GUN_2_SAAT,
            limit: KONTENJAN_GUN_2,
...
function initStats() {
    ...
    sheet.getRange(STAT_ROW_KALAN, STAT_COL_GUN_1).setValue(KONTENJAN_GUN_1);

    // Gun 2
    sheet.getRange(STAT_ROW_GIRIS, STAT_COL_GUN_2).setValue(0);
    sheet.getRange(STAT_ROW_ONAYLI, STAT_COL_GUN_2).setValue(0);
    sheet.getRange(STAT_ROW_KALAN, STAT_COL_GUN_2).setValue(KONTENJAN_GUN_2);
...
function countBiletler() {
    ...
    statSheet.getRange(STAT_ROW_KALAN, STAT_COL_GUN_1).setValue(KONTENJAN_GUN_1 - onayliGun1);
    ...
    statSheet.getRange(STAT_ROW_KALAN, STAT_COL_GUN_2).setValue(KONTENJAN_GUN_2 - onayliGun2);

    Logger.log('=== COUNT SONUCLARI ===');
    Logger.log(ETKINLIK_GUN_1_LABEL + ' - Onayli: ' + onayliGun1 + ', Giris: ' + girisGun1 + ', Kalan: ' + (KONTENJAN_GUN_1 - onayliGun1));
    Logger.log(ETKINLIK_GUN_2_LABEL + ' - Onayli: ' + onayliGun2 + ', Giris: ' + girisGun2 + ', Kalan: ' + (KONTENJAN_GUN_2 - onayliGun2));

    return {
        gun1: { onayli: onayliGun1, giris: girisGun1, kalan: KONTENJAN_GUN_1 - onayliGun1 },
        gun2: { onayli: onayliGun2, giris: girisGun2, kalan: KONTENJAN_GUN_2 - onayliGun2 }
    };
}
...
function sendReminderGun1() {
    sendReminderByDate_(ETKINLIK_GUN_1_TOKEN, ETKINLIK_GUN_1_SAAT);
}
...
function sendReminderGun2() {
    sendReminderByDate_(ETKINLIK_GUN_2_TOKEN, ETKINLIK_GUN_2_SAAT);
}
...
function getStatsForApi(gun) {
    try {
        ...
        const col = (gun === ETKINLIK_GUN_1_TOKEN) ? STAT_COL_GUN_1 : STAT_COL_GUN_2;
    ...
    return {
        ...
        stats: {
            ...
            gun1: {
                ...
                giris: girisGun1,
                kalan: KONTENJAN_GUN_1 - odemeOnayliGun1,
                doluluk: Math.round((odemeOnayliGun1 / KONTENJAN_GUN_1) * 100)
            },
            gun2: {
                label: ETKINLIK_GUN_2_LABEL,
                token: ETKINLIK_GUN_2_TOKEN,
                ...
                giris: girisGun2,
                kalan: KONTENJAN_GUN_2 - odemeOnayliGun2,
                doluluk: Math.round((odemeOnayliGun2 / KONTENJAN_GUN_2) * 100)
            }
        }
    };
}
